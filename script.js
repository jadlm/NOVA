...
window.loadHomepage = async function() {
  ...
  container.innerHTML = `
    ...
    <div class="max-w-7xl mx-auto px-4">
      ...
      <div class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl rounded-3xl p-8 text-center my-16 floating"><h3 class="text-2xl font-bold">🚚 Livraison gratuite dès 990 MAD</h3><button class="mt-4 ripple-btn btn-shine px-8 py-3 text-lg" onclick="window.location.href='shop.html'">Explorer la boutique →</button></div>
  ...
  grid.innerHTML = featured.map((p, index) => `<div class="product-card bg-white/90 dark:bg-gray-800/90 backdrop-blur-lg rounded-2xl overflow-hidden shadow-lg" style="animation-delay: ${index * 0.1}s"><img src="${p.images[0]}" class="h-56 w-full object-cover"><div class="p-4"><h3 class="font-bold text-lg">${p.name}</h3><p class="text-sm text-gray-600 dark:text-gray-400 mb-2">${p.description}</p><div class="flex items-center justify-between mb-3"><p class="text-lg font-semibold text-blue-600 dark:text-blue-400">${p.price} MAD</p><div class="flex items-center gap-1"><span class="text-yellow-500">⭐</span><span class="text-sm font-medium">${p.rating}</span><span class="text-xs text-gray-500">(${p.reviews})</span></div></div><div class="flex gap-2"><button onclick="addToCart(${p.id},1)" class="flex-1 ripple-btn bg-blue-600 hover:bg-blue-700 text-white py-2 rounded-lg">🛒 Ajouter</button><button onclick="window.location.href='product.html?id=${p.id}'" class="flex-1 border-2 border-gray-300 dark:border-gray-600 py-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition">👁️ Détail</button></div></div></div>`).join('');
